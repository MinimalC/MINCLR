...
export System_Exception  System_Exception_new(System_string8 message);
...
#define inline_System_Exception_new(MESSAGE)  (base_System_Exception_init(inline_System_Object_allocClass(System_Exception), MESSAGE))
...
#define catch_class(TYPE,THAT,ACTION)  TYPE THAT = inline_System_Object_asType(System_Exception_current, TYPE);\
    if (THAT) { System_Object_addReference((System_Object)THAT); System_Exception_set_current(null); ACTION ; inline_System_Object_freeClass(&THAT); }
