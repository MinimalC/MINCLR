...
int CubeHash512_main(int argc, char * argv[]) {

    if (argc == 1) {
        System_Console_writeLine("Use CubeHash512 \"Hallo Welt\" or specify --file HalloWelt.txt", 0);
    ...
    for (Size i = 1; i < argc; ++i) {
        Stack_clear(cubeHash512); CubeHash512_init(&cubeHash512);
