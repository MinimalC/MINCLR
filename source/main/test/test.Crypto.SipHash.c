...
int main(int argc, char * argv[]) {
    ...
    if (argc > 1) {
        ...
        if (!hash_value || hash_value != SipHash48_HalloWelt)
            __Console_writeLine("Test02: ERROR:         Crypto_SipHash48: 0x{0:uint:hex}", 1, hash_value);
        else
            __Console_writeLine("Test02: SUCCESS:         Crypto_SipHash48: 0x{0:uint:hex}", 1, hash_value);
    }
