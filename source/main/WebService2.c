...
Network_HTTPResponse HTTPService_request(System_String message) {
...
IntPtr HTTPService_serve(Size argc, Var argv[]) {
    ...
    while (true) {
        ...
        if (poller & Network_PollFlags_IN) {
            System_String message = base_Network_TCPSocket_receive(tcp);
            ...
            response = HTTPService_request(message);
        }
